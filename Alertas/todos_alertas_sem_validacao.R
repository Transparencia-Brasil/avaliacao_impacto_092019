...
todos_alertas <- inspections %>%
  ...
  select(inspection_id , user_id, project_id, created_at.x, updated_at.x , 
         respondido, content, alerta_campanha, 'Responsável', city_id.y, funded_by) %>%
  ...
  select(inspection_id, campaign_alert , inspection_created_at, 
         ...
         location_city, state, 
         inspection_status, entity_who_answered, answer_content) 


setwd("C:/Users/coliv/Documents/avaliacao_impacto_092019/Alertas")
save(todos_alertas, file="todos_alertas_sem_validacao.Rdata")
