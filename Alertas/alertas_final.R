...
# Nome: validacao_respostas_final
load("C:/Users/coliv/Documents/avaliacao_impacto_092019/Validação de respostas/validacao_respostas_final.Rdata")

not_valid_answers <- validacao_respostas_final %>%
  filter(resposta_valida == 0)

not_valid_answers <- unique(not_valid_answers$id)
...
todos_alertas <- inspections %>%
  ...
  mutate(alerta_campanha = ifelse(user_id == 5977 |                
         ...
         alerta_campanha_coordenadas = ifelse(user_id == 7374, 1, 0), 
         alerta_campanha_coordenadas = ifelse(is.na(user_id), 0, alerta_campanha_coordenadas),
  ...
  select(inspection_id , user_id, project_id, created_at.x, updated_at.x , 
         respondido, content, alerta_campanha, 'Responsável', city_id.y, funded_by, valid_answer, alerta_campanha_coordenadas) %>%
  rename(inspection_created_at = created_at.x,
         ...
         coordinates_campaign = alerta_campanha_coordenadas) %>%
  ...
  rename(location_city = name.x,
         state = abbreviation) %>%
  filter(inspection_created_at < "2019-07-31") %>%    #estabelecendo a data final
  left_join(inspection_instancias, by=c("inspection_id") ) %>%
  select(inspection_id, campaign_alert , coordinates_campaign, inspection_created_at, 
...
setwd("C:/Users/coliv/Documents/avaliacao_impacto_092019/Alertas")
